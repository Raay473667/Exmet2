...
def two_gaussians(x, a1, sigma1, a2, mu1, mu2, a3, a4):
    fwhm = 2*np.sqrt(2*np.log(2))*sigma1
    gauss1 = a1 * np.exp(-(x - mu1)**2 / (2 * fwhm**2))
    gauss2 = a2 * np.exp(-(x - mu2)**2 / (2 * fwhm**2))

    lorentz1 =  a3 * (sigma1 / 2)**2 / ((x - mu1)**2 + (sigma1 / 2)**2)
    lorentz2 =  a4 * (sigma1 / 2)**2 / ((x - mu2)**2 + (sigma1 / 2)**2)
    return gauss1 + gauss2 + lorentz1 + lorentz2
...
def first_gaussian(x, a1, sigma1, mu1, a3):
    fwhm = 2*np.sqrt(2*np.log(2))*sigma1
    gauss1 = a1 * np.exp(-(x - mu1)**2 / (2 * fwhm**2))
    lorentz1 =  a3 * (sigma1 / 2)**2 / ((x - mu1)**2 + (sigma1 / 2)**2)
    return gauss1 + lorentz1
    return lorentz1
    return gauss1

def second_gaussian(x, a2, sigma2, mu2, a4):
    fwhm = 2*np.sqrt(2*np.log(2))*sigma2
    gauss2 = a2 * np.exp(-(x - mu2)**2 / (2 * fwhm**2))
    lorentz2 =  a4 * (sigma2 / 2)**2 / ((x - mu2)**2 + (sigma2 / 2)**2)
    return gauss2 + lorentz2
    return lorentz2
    return gauss2
...
def plot(x01, x02, p0, bounds, y_plot):
    ...
    crystal_size = 0.94 * K_alfa_1 / np.radians(fwhm/2) / np.cos(np.radians(params[3]/2))
    print(f'crystal_size = {crystal_size:.2f}')
    y_plot = y_plot + two_gaussians(x, *params)
    fig, ax = plt.subplots()
    ax.plot(x, y, label = 'Measured Data')
    ax.plot(x, first_gaussian(x, params[0], params[1], params[3], params[5]), label="K$_{α1}$ Gaussian")
    ax.plot(x, second_gaussian(x, params[2], params[1], params[4], params[6]), label="K$_{α2}$ Gaussian")
    ax.plot(x, two_gaussians(x, *params), label="K$_{α1}$ + K$_{α2}$ Gaussian")
...
def main():
    ...
    x01 = 43.14
    x02 = 43.25
    p0 = [50000, 0.02, 20000, x01, x02, 10000, 5000]
    #bounds = ([20000, 0, 10000, 0, 0, 0, 0], [np.inf, 0.1, np.inf, 100, 100, np.inf, np.inf])
    bounds = ([20000, 0, 1000, 0, 0, 0, 0], [np.inf, 0.2, np.inf, 100, 100, np.inf, np.inf])
    plot(x01, x02, p0, bounds, y_plot)
    
    x01 = 50.28
    x02 = 50.39
    p0 = [100000, 0.1, 50000, x01, x02, 10000, 5000]
    bounds = ([0, 0, 10000, 0, 0, 0, 0], [np.inf, 0.2, np.inf, 100, 100, np.inf, np.inf])
    plot(x01, x02, p0, bounds, y_plot)
    
    x01 = 73.98
    x02 = 74.19
    p0 = [40000, 0.1, 20000, x01, x02, 10000, 5000]
    bounds = ([0, 0, 0, 0, 0, 0, 0], [np.inf, 0.2, np.inf, 100, 100, np.inf, np.inf])
    plot(x01, x02, p0, bounds, y_plot)
    
    x01 = 89.80
    x02 = 90.08
    p0 = [25000, 0.06, 12000, x01, x02, 10000, 5000]
    #bounds = ([0, 0, 0, 0, 0, 0, 0], [np.inf, 0.2, np.inf, 100, 100, np.inf, np.inf])
    bounds = ([0, 0, 0, 0, 0, 0, 0], [np.inf, 0.3, np.inf, 100, 100, np.inf, np.inf])
    plot(x01, x02, p0, bounds, y_plot)
    
    x01 = 95.0
    x02 = 95.4
    y = y + 300
    p0 = [2000, 0.08, 1000, x01, x02, 10000, 5000]
    bounds = ([0, 0, 0, 0, 0, 0, 0], [np.inf, 0.2, np.inf, 100, 100, np.inf, np.inf])
    plot(x01, x02, p0, bounds, y_plot)
    y = y - 300
    ...
    fig, ax = plt.subplots()
    ax.plot(x, y, label = 'Measured Data')
    ax.plot(x, y_plot, label="K$_{α1}$ + K$_{α2}$ Gaussian")
    ax.xaxis.set_minor_locator(AutoMinorLocator(4))
    ax.yaxis.set_minor_locator(AutoMinorLocator(5))
    ax.set_xlabel("$2θ$ [°]")
    ax.set_ylabel("Intensity [a.u.]")
